...
public:
     ...
     {
          return airport_[ static_cast< std::size_t >( airport ) ];
     }

     // добавить данный элемент
     void Insert( TAirport airport )
     {
          airport_[ static_cast< std::size_t >( airport ) ]++;
     }
     ...
     {
          airport_[ static_cast< std::size_t >( airport ) ] = 0;
     }
     ...
     {
          Items items;
          for( std::size_t i = 0; i < airport_.size(); ++i )
          {
               items[ i ] = { static_cast< TAirport >( i ), airport_[ i ] };
          }
          return items;
     }

private:
     std::array< size_t, static_cast< std::size_t >( TAirport::Last_ ) > airport_;
};
...
void TestManyConstructions()
{
     ...
     ASSERT( total < 1000 );
     std::cout << "TestManyConstructions: " << total << '\n';
}
...
void TestManyGetItems()
{
     ...
     // Assert to use variable total so that compiler doesn't optimize it out
     ASSERT( total > 0 );
     std::cout << "TestManyGetItems: " << total << '\n';
}
